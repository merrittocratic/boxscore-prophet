...
# prior_vol_stats is built straight off te_plays (never filtered to
# !is.na(player_id) the way te_outcomes is above), so its NA-player
# pseudo-row (uncredited targets per team-game) is a real, if incidental,
# member of the udfa tier median -- te_draft below carries a matching
...
# than the tidied te_outcomes above) is required for an exact gate match.
te_outcomes_raw <- readRDS("data/te_outcomes.rds")
...
#     12a_te_feature_layer.R v1.1 Step 7b: real prior-season share ->
...
#     just TE) come straight from te_outcomes.rds's team_total_targets/
...
prior_vol_stats <- te_outcomes_raw |>
  filter(season == TARGET_SEASON - 1L) |>
  group_by(player_id) |>
  summarise(
...
# 12a_te_feature_layer.R's Step 5 team_plays_obs (ALL scrimmage plays from
# raw pbp, position-agnostic) + Step 7b's prior_team_plays_vol. te_outcomes
# is TE-presence-filtered (one row per TE target), so a team-game where the
# TE(s) went scoreless on targets (a real, documented blocking-game gap --
# 26.5% of high-snap TE weeks fall under the target floor, per feasibility
...
# Prior-season snap-share AND targets-per-snap carryforward -- same fallback
# ladder as baseline_target_share above (real prior-season value -> tier
# median -> league median). tgt_per_snap uses a pooled season-level ratio
# (sum targets / sum snaps), matching 12a step 8 (not a mean of noisy
# per-game ratios). Needs TARGET_SEASON-1 snap data, which the
# TARGET_SEASON-only snaps_raw pull above doesn't have; TARGET_SEASON-1 is
# always a completed season so this pull can't hit the pre-season loader
# guard for real, but load_season_or_empty is used anyway for defensive
# consistency.
...
snaps_prior_clean <- snaps_prior_raw |>
  ...
  left_join(id_xwalk, by = c("pfr_player_id" = "pfr_id")) |>
  filter(!is.na(gsis_id)) |>
  select(gsis_id, week, snap_pct, offense_snaps)

game_targets_prior <- te_outcomes |>
  filter(season == TARGET_SEASON - 1L) |>
  select(player_id, week, targets)

prior_snap_stats <- snaps_prior_clean |>
  left_join(game_targets_prior, by = c("gsis_id" = "player_id", "week")) |>
  mutate(targets_filled = coalesce(targets, 0L)) |>
  group_by(gsis_id) |>
  summarise(
  ...
  mutate(prior_tgt_per_snap = if_else(prior_snaps_sum > 0L,
                                      prior_targets_sum / prior_snaps_sum, NA_real_)) |>
  select(player_id = gsis_id, prior_snap_share, prior_tgt_per_snap)
...
pos_tgt_per_snap <- median(prior_snap_stats$prior_tgt_per_snap, na.rm = TRUE)

snap_vol_const <- te_draft |>
  ...
  select(player_id, baseline_snap_share, baseline_tgt_per_snap)

n_na_bss  <- sum(is.na(snap_vol_const$baseline_snap_share))
n_na_btps <- sum(is.na(snap_vol_const$baseline_tgt_per_snap))
cli_alert_success("baseline_snap_share built | NA: {n_na_bss} of {nrow(snap_vol_const)}")
cli_alert_success("baseline_tgt_per_snap built | NA: {n_na_btps} of {nrow(snap_vol_const)}")
