# R/12b_te_lgbm_tuned.R
# Step 4b: Nested-walk-forward-tuned LightGBM for TE EPA model.
#
# CLONES 04b_wr_lgbm_tuned.R with TE-specific adjustments:
#   - VOL_FEATURES: adds wt_tgt_per_snap (role signal from 12a; separates
#     route-running TEs from blockers -- feasibility receipt: 26.5% of
#     high-snap TE weeks fall under the 3-target floor vs 10.7% WR)
#   - Reads data/te_feature_table.rds; defensive split at 7 air yards (12a)
#   - Low-opp bucket: 3-5 targets (same floor as WR)
#   - EXPECTED_TEST_N computed from TE feature table
#
...
# TE volume features: WR set + wt_tgt_per_snap (role signal -- separates
# route-running TEs from blockers whose snaps do not convert to targets)
VOL_FEATURES <- c(
...
cli_h1("Step 12b: TE Nested-Walk-Forward-Tuned LightGBM")
...
# EXPECTED_TEST_N computed from TE feature table -- WR row count per fold differs from RB
EXPECTED_TEST_N <- ft |>
  semi_join(fold_map, by = c("season" = "test_season", "week" = "test_week")) |>
  nrow()

cli_alert_success("TE feature table: {nrow(ft)} rows | Fold map: {nrow(fold_map)} folds | Expected test rows: {EXPECTED_TEST_N}")
...
cli_alert_success("All model features present in TE feature table")
...
cli_h1("12b-TE Pooled Coverage (all {n_scored} test rows)")
...
cli_h1("12b-TE Low-Usage Bucket (targets {LOW_OPP_LO}-{LOW_OPP_HI})")
...
cli_h1("Rubric Decision: 12b-TE")

te_pool80 <- pooled_te |> filter(component == "combined", nominal == 0.80)
te_low80  <- low_te    |> filter(component == "combined", nominal == 0.80)

cli_alert_info("Pooled 80%: delta={fmt_pp(te_pool80$delta)} | width={fmt_w(te_pool80$sharpness)}")
cli_alert_info("Low-opp 80%: delta={fmt_pp(te_low80$delta)} | width={fmt_w(te_low80$sharpness)}")

if (abs(te_low80$delta) > 0.10) {
  cli_warn("12b-TE VETOED: low-usage 80% delta = {fmt_pp(te_low80$delta)} (threshold: +-10pp)")
} else {
  cli_alert_success("12b-TE veto passed: {fmt_pp(te_low80$delta)}")
  if (abs(te_pool80$delta) <= 0.02) {
    cli_alert_success("Pooled coverage within +-2pp -- TE model READY")
  } else {
    cli_alert_info("Pooled coverage {fmt_pp(te_pool80$delta)} -- outside +-2pp; inspect stratified table")
  }
}
...
readr::write_csv(results,  "output/12b_te_lgbm_fold_predictions.csv")
readr::write_csv(pooled_te, "output/12b_te_lgbm_pooled_coverage.csv")
readr::write_csv(low_te,    "output/12b_te_lgbm_low_usage_coverage.csv")
readr::write_csv(tune_all,  "output/12b_te_lgbm_tune_log.csv")

cli_alert_success("output/12b_te_lgbm_fold_predictions.csv  ({nrow(results)} rows)")
cli_alert_success("output/12b_te_lgbm_pooled_coverage.csv")
cli_alert_success("output/12b_te_lgbm_low_usage_coverage.csv")
cli_alert_success("output/12b_te_lgbm_tune_log.csv  (full per-fold param audit)")

cli_h1("Step 12b complete")
