# R/12e_te_recalibration.R
# Step 12e: Walk-forward recalibration of P(FP >= 12) and P(FP >= 17) -- TE.
#
# TE clone of 06c: same five-method bake-off (platt / iso / strat_platt /
# strat_iso / platt_vol), same PRE-COMMITTED judge rule, same walk-forward
# honesty discipline. See the 06c header for the full motivation trail
# (2026-07-05 compounding-effects diagnosis; 2026-07-17 volume-conditional
# extension that resolved the WR streamer/high-volume conditional bias).
#
# TE-SPECIFIC SETTINGS:
#   - Thresholds 12+/17+ (rate-matched cuts from 12_te_feasibility.R)
#   - Ex-ante volume strata at pred_vol 4 / 7 targets: streamer TE (<=4),
#     mid (5-7), locked-in (8+). Chosen from the TE target distribution;
#     MIN_STRAT_N=300 pooled fallback guards thin strata exactly as in 06c.
#     Volume-model shrinkage may compress pred_vol (the 06f RB lesson) --
#     stratum counts are printed and must be checked before trusting picks.
#
# DESIGN (mirrors 06c):
#   - Input: output/12d_te_fp_sim_probabilities.csv (product config:
#     spline translation + 12c asymmetric TE, pred-vol rescaled)
#   - Maps: TE x threshold (12+/17+)
...
#   - Deployment: winning-method maps refit on ALL data -> data/te_fp_recal_maps.rds
...
STRATA_BREAKS <- list(TE = c(-Inf, 4, 7, Inf))
...
cli_h1("Step 12e: Walk-forward recalibration (volume-conditional bake-off)")

probs <- readr::read_csv("output/12d_te_fp_sim_probabilities.csv", show_col_types = FALSE) |>
...
col_for <- function(thresh, method) {
  stem <- if (thresh == "12+") "p_start" else "p_boom"
...
cal_all <- pmap(grid, function(pos, thresh, method) {
  calibrate(recal, col_for(thresh, method),
            if (thresh == "12+") "hit_start" else "hit_boom",
            method, thresh, pos)
}) |> list_rbind()
...
brier <- pmap(grid, function(pos, thresh, method) {
  ...
  h <- as.numeric(d[[if (thresh == "12+") "hit_start" else "hit_boom"]])
...
strata_tbl <- pmap(picks, function(position, threshold, pick, reason) {
  hit_col <- if (threshold == "12+") "hit_start" else "hit_boom"
...
deploy_maps <- pmap(picks, function(position, threshold, pick, reason) {
  ...
  p <- d[[if (threshold == "12+") "p_start" else "p_boom"]]
  h <- d[[if (threshold == "12+") "hit_start" else "hit_boom"]]
  s <- d$stratum
  v <- d$pred_vol
...
saveRDS(deploy_maps, "data/te_fp_recal_maps.rds")
...
readr::write_csv(recal,     "output/12e_te_recal_probabilities.csv")
readr::write_csv(cal_all,   "output/12e_te_recal_calibration.csv")
readr::write_csv(cal_strat, "output/12e_te_recal_calibration_strat.csv")
readr::write_csv(summary_tbl |>
                 ...
                 "output/12e_te_recal_summary.csv")
readr::write_csv(strata_tbl, "output/12e_te_recal_strata.csv")
readr::write_csv(map_grid,   "output/12e_te_recal_map_grid.csv")

cli_alert_success("output/12e_te_recal_probabilities.csv ({nrow(recal)} rows)")
cli_alert_success("output/12e_te_recal_calibration.csv")
cli_alert_success("output/12e_te_recal_calibration_strat.csv")
cli_alert_success("output/12e_te_recal_summary.csv")
cli_alert_success("output/12e_te_recal_strata.csv")
cli_alert_success("output/12e_te_recal_map_grid.csv")
cli_alert_success("data/te_fp_recal_maps.rds (deployment maps)")

cli_h1("Step 12e complete")
