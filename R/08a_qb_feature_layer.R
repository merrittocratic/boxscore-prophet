...
# production trace.
SNAP_SEASONS <- intersect(PREDICTION_SEASONS, PBP_SEASONS)
REQ_SNAP_COLS <- c("pfr_player_id", "season", "week", "game_type", "offense_pct")
snaps_raw <- map(SNAP_SEASONS, function(s) {
...
if (nrow(snaps_raw) == 0) {
  cli_abort("No snap count data fetched for ANY season in {paste(range(SNAP_SEASONS), collapse='-')} -- this is a real failure (network/nflreadr issue), not the expected single-new-season gap")
}
cli_alert_success("Snap counts: {n_distinct(snaps_raw$season)} of {length(SNAP_SEASONS)} requested seasons returned data")
