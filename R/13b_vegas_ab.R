...
#
# Usage: Rscript R/13b_vegas_ab.R RB|WR|TE
#
...
#   - ARM A = the SHIPPED fold predictions (11c RB / 04c WR / 12c TE).
#     Not re-run; loaded from the frozen receipts.
...
#     volume component is REFIT from the shipped per-fold tune log (seed
#     42, deterministic) and must reproduce shipped pred_vol EXACTLY
...
# ---------------------------------------------------------------------------
# Position config (feature sets copied verbatim from 10a deployment sets)
# ---------------------------------------------------------------------------
...
CFG <- list(
  RB = list(
    ...
    preds     = "output/11c_rb_injury_fold_predictions.csv",
    ...
    vol = c("wt_carry_share", "wt_target_share", "wt_snap_share",
            "wt_team_total_plays", "def_rush_epa_adj", "draft_tier_int",
            "is_cold_start_int", "games_played_so_far", RB_INJURY_FEATURES)
  ),
  WR = list(
    table     = "data/wr_feature_table.rds",
    preds     = "output/04c_wr_asym_fold_predictions.csv",
    ...
    eff = c("prior_epa_per_opp", "baseline_epa_per_opp", "rolling_epa_per_opp",
            ...
            "wt_snap_share", "games_played_so_far", "def_used_fallback_int"),
    vol = c("wt_target_share", "wt_air_yards_share", "wt_snap_share",
            "wt_team_total_plays", "def_short_pass_epa_adj",
            "def_deep_pass_epa_adj", "draft_tier_int", "is_cold_start_int",
            "games_played_so_far")
  ),
  TE = list(
    table     = "data/te_feature_table.rds",
    preds     = "output/12c_te_asym_fold_predictions.csv",
    ...
    eff = c("prior_epa_per_opp", "baseline_epa_per_opp", "rolling_epa_per_opp",
            ...
            "wt_snap_share", "games_played_so_far", "def_used_fallback_int"),
    vol = c("wt_target_share", "wt_air_yards_share", "wt_snap_share",
            "wt_tgt_per_snap", "wt_team_total_plays", "def_short_pass_epa_adj",
            "def_deep_pass_epa_adj", "draft_tier_int", "is_cold_start_int",
            "games_played_so_far")
  )
)
...
cli_h1("13b Vegas A/B -- {POS} (arm B: EFF + {paste(VEGAS_FEATURES, collapse=', ')})")
...
EXPECTED_TEST_N <- ft |>
  ...
  nrow()
cli_alert_success("{POS}: {nrow(ft)} rows | {nrow(fold_map)} folds | expected test rows {EXPECTED_TEST_N} | shipped arm rows {nrow(shipped)}")

# ===========================================================================
# 2. WALK-FORWARD LOOP (arm B: retune EFF, refit VOL from shipped params)
# ===========================================================================
...
for (f in seq_len(nrow(fold_map))) {
  ...
  # --- VOL: refit from shipped tune log (deterministic reproduction) ---
  tl <- tune_log[f, ]
...
cli_alert_info("VOL reproduction: {nrow(vol_check)} matched rows, max |pred_vol diff| = {format(max_vol_diff, scientific = TRUE)}")
if (max_vol_diff > 1e-6) cli_abort("VOL arm failed to reproduce shipped predictions -- procedure drift, do not trust the A/B.")
...
cli_h1("13b verdict -- {POS} (pre-committed rule)")
...
stem <- sprintf("output/13b_%s_vegas%s", tolower(POS),
