# R/04c_wr_asymmetric_conformal_notier.R
# Step 4c NOTIER CLONE: WR asymmetric-conformal retrain with draft_tier_int
# removed from EFF_FEATURES (efficiency model only).
#
# CLONE DISCIPLINE: this file is 04c_wr_asymmetric_conformal_volfix.R
# verbatim except for (1) this header, (2) "draft_tier_int" dropped from
# EFF_FEATURES, and (3) output paths get a _notier suffix so neither the
# frozen 04c outputs nor the volfix outputs are overwritten. VOL_FEATURES
# (including the volfix baseline_* carryforward addition), the fold map,
# the fit/cal split logic, the reused 04b hyperparameters, and the
# asymmetric conformal construction are all untouched.
#
# WHY: a 204-fold counterfactual audit found draft_tier_int causes real,
# meaningful efficiency-prediction swings (5-14.7%) for specific real
# elite-but-low-draft-pedigree players in ~15-30% of folds, purely from
# tier, holding actual performance data fixed -- even though its aggregate
# gain-based importance is always small (<~1.9%, typically <0.3%). This
# retrain drops the feature entirely (rather than perturbing it) to see
# whether the same per-fold hyperparameters (tuned WITH the feature
# present, reused unchanged here) still produce a sane model without it,
# and whether the flagged-player swing instability actually goes away.
#
# BASE CHOICE: built on top of 04c_wr_asymmetric_conformal_volfix.R (not
# the original 04c) so VOL_FEATURES carries the baseline_* Week-1 fix.
# EFF_FEATURES minus draft_tier_int is the only efficiency-side change, so
# pred_eff/med_eff here are comparable apples-to-apples against BOTH the
# original 04c (pred_eff is identical between original and volfix, since
# EFF_FEATURES never differed between those two) and against volfix
# (isolates draft_tier_int alone). The combined ("tot") component,
# however, inherits the volfix volume features, so combined/pooled
# comparisons against the ORIGINAL (non-volfix) 04c conflate two changes
# -- see the companion comparison analysis for how that is handled.
#
...
EFF_FEATURES <- c(
  ...
  "is_cold_start_int",
  # NOTIER: draft_tier_int removed -- see script header for the 204-fold
  # counterfactual-swing finding that motivated this retrain.
...
cli_h1("Step 4c NOTIER: WR Asymmetric Conformal (draft_tier_int dropped from EFF_FEATURES)")
...
cli_h1("Save outputs (NOTIER -- new files, originals + volfix untouched)")

readr::write_csv(results,   "output/04c_wr_asym_fold_predictions_notier.csv")
readr::write_csv(pooled_wr, "output/04c_wr_asym_pooled_coverage_notier.csv")
readr::write_csv(low_wr,    "output/04c_wr_asym_low_usage_coverage_notier.csv")

cli_alert_success("output/04c_wr_asym_fold_predictions_notier.csv ({nrow(results)} rows)")
cli_alert_success("output/04c_wr_asym_pooled_coverage_notier.csv")
cli_alert_success("output/04c_wr_asym_low_usage_coverage_notier.csv")

cli_h1("Step 4c NOTIER complete")
