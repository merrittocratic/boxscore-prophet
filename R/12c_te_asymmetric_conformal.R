# R/12c_te_asymmetric_conformal.R
...
cli_h1("Step 12c: TE Asymmetric Conformal (04b learner, signed-residual intervals)")
...
cli_h1("12c-TE Low-Usage Bucket (targets {LOW_OPP_LO}-{LOW_OPP_HI})")
...
cli_h1("Rubric Decision: 12c vs 12b")

te_pool80 <- pooled_te |> filter(component == "combined", nominal == 0.80)
te_low80  <- low_te    |> filter(component == "combined", nominal == 0.80)

b_pool <- readr::read_csv("output/12b_te_lgbm_pooled_coverage.csv", show_col_types = FALSE) |>
  filter(component == "combined", nominal == 0.80)
b_low  <- readr::read_csv("output/12b_te_lgbm_low_usage_coverage.csv", show_col_types = FALSE) |>
  filter(component == "combined", nominal == 0.80)

cli_alert_info("12b (symmetric):  pooled {fmt_pp(b_pool$delta)} w={fmt_w(b_pool$sharpness)} | low {fmt_pp(b_low$delta)}")
cli_alert_info("12c (asymmetric): pooled {fmt_pp(te_pool80$delta)} w={fmt_w(te_pool80$sharpness)} | low {fmt_pp(te_low80$delta)}")

if (abs(te_low80$delta) > 0.10) {
  cli_warn("12c VETOED: low-usage 80% delta = {fmt_pp(te_low80$delta)}")
} else if (abs(te_pool80$delta) > 0.02) {
  cli_warn("12c pooled 80% outside +-2pp: {fmt_pp(te_pool80$delta)}")
} else {
  cli_alert_success("12c passes rubric: pooled {fmt_pp(te_pool80$delta)}, low {fmt_pp(te_low80$delta)}")
}
...
readr::write_csv(results,   "output/12c_te_asym_fold_predictions.csv")
readr::write_csv(pooled_te, "output/12c_te_asym_pooled_coverage.csv")
readr::write_csv(low_te,    "output/12c_te_asym_low_usage_coverage.csv")

cli_alert_success("output/12c_te_asym_fold_predictions.csv ({nrow(results)} rows)")
cli_alert_success("output/12c_te_asym_pooled_coverage.csv")
cli_alert_success("output/12c_te_asym_low_usage_coverage.csv")

cli_h1("Step 12c complete")
